...
--
-- Host: localhost
-- Generation Time: Oct 14, 2019 at 05:41 AM
...
-- Database: `info`
--
CREATE DATABASE IF NOT EXISTS `info` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `info`;
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `SearchAge`(IN `page` INT(11))
SELECT player_name,age,overall_rating,nationality FROM personal_details WHERE personal_details.age = page$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SearchName`(IN `page` VARCHAR(30))
SELECT * FROM personal_details WHERE player_name = page$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SearchNationality`(IN `page` VARCHAR(30))
SELECT * FROM personal_details WHERE nationality=page$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SearchOverallRating`(IN `page` INT(11))
SELECT * FROM personal_details WHERE personal_details.overall_rating = page$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Searchplayerid`(IN `page` INT(11))
SELECT * FROM personal_details WHERE player_id = page$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SearchPosition`(IN `page` VARCHAR(11))
SELECT pd.player_name, pd.overall_rating, od.preferred_position, p.gk, p.df, p.cm, p.fr FROM personal_details pd, other_details od, position p WHERE od.preferred_position = page AND p.player_id = od.player_id AND p.player_id = pd.player_id GROUP BY pd.player_id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SearchTeam`(IN `page` VARCHAR(30))
SELECT pd.player_name,pd.overall_rating,pd.age,pd.nationality,od.club FROM personal_details pd,other_details od WHERE od.club = page AND pd.player_id = od.player_id ORDER BY pd.player_id$$
...
(30, 'Inserted Successfully in PERSONAL DETAILS Table', '2019-10-12 13:05:24'),
(31, 'Inserted Successfully in PERSONAL DETAILS Table', '2019-10-12 13:05:24');
...
DELIMITER //
CREATE TRIGGER `delete_in_otherdetails` AFTER DELETE ON `other_details`
 FOR EACH ROW INSERT INTO delete_logs(action,time) VALUES ("Deleted Successfully in PLAYER CLUB'S Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `insert_in_otherdetails`;
DELIMITER //
CREATE TRIGGER `insert_in_otherdetails` AFTER INSERT ON `other_details`
 FOR EACH ROW INSERT INTO insert_logs(action,time) VALUES ("Inserted Successfully in PLAYER CLUB'S Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `update_in_otherdetails`;
DELIMITER //
CREATE TRIGGER `update_in_otherdetails` AFTER UPDATE ON `other_details`
 FOR EACH ROW INSERT INTO update_logs VALUES(null,"Updated Successfully in CLUB's Table",NOW())
//
DELIMITER ;
...
DELIMITER //
CREATE TRIGGER `delete_trigger` AFTER DELETE ON `personal_details`
 FOR EACH ROW INSERT INTO delete_logs(action,time) VALUES ("Deleted Successfully in PERSONAL DETAILS Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `insert_trigger`;
DELIMITER //
CREATE TRIGGER `insert_trigger` AFTER INSERT ON `personal_details`
 FOR EACH ROW INSERT INTO insert_logs VALUES (null,"Inserted Successfully in PERSONAL DETAILS Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `update_trigger`;
DELIMITER //
CREATE TRIGGER `update_trigger` AFTER UPDATE ON `personal_details`
 FOR EACH ROW INSERT INTO update_logs(action,time) VALUES ("Updated Successfully in PERSONAL DETAILS Table",NOW())
//
DELIMITER ;
...
DELIMITER //
CREATE TRIGGER `delete_in_playerstats` AFTER DELETE ON `player_stats`
 FOR EACH ROW INSERT INTO delete_logs(action,time) VALUES ("Deleted Successfully in PLAYER STATS Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `insert_trigger_in_playerstats`;
DELIMITER //
CREATE TRIGGER `insert_trigger_in_playerstats` AFTER INSERT ON `player_stats`
 FOR EACH ROW INSERT INTO insert_logs(action,time) VALUES ("Inserted Successfully in PLAYER STATS Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `update_in_player_stats`;
DELIMITER //
CREATE TRIGGER `update_in_player_stats` AFTER UPDATE ON `player_stats`
 FOR EACH ROW INSERT INTO update_logs VALUES(null,"Updated Successfully in PLAYER STATS Table",NOW())
//
DELIMITER ;
...
DELIMITER //
CREATE TRIGGER `delete_in_position` AFTER DELETE ON `position`
 FOR EACH ROW INSERT INTO delete_logs(action,time) VALUES ("Deleted Successfully in PLAYER'S POSITION Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `insert_in_playerposition`;
DELIMITER //
CREATE TRIGGER `insert_in_playerposition` AFTER INSERT ON `position`
 FOR EACH ROW INSERT INTO insert_logs(action,time) VALUES ("Inserted Successfully in PLAYER'S POSITION Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `update_in_position`;
DELIMITER //
CREATE TRIGGER `update_in_position` AFTER UPDATE ON `position`
 FOR EACH ROW INSERT INTO update_logs VALUES(null,"Updated Successfully in PLAYER'S POSITION Table",NOW())
//
DELIMITER ;
...
DELIMITER //
CREATE TRIGGER `delete_in_salary` AFTER DELETE ON `salary`
 FOR EACH ROW INSERT INTO delete_logs(action,time) VALUES ("Deleted Successfully in PLAYER'S SALARY Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `insert_in_salary`;
DELIMITER //
CREATE TRIGGER `insert_in_salary` AFTER INSERT ON `salary`
 FOR EACH ROW INSERT INTO insert_logs(action,time) VALUES ("Inserted Successfully in PLAYER SALARY Table",NOW())
//
DELIMITER ;
DROP TRIGGER IF EXISTS `update_in_salary`;
DELIMITER //
CREATE TRIGGER `update_in_salary` AFTER UPDATE ON `salary`
 FOR EACH ROW INSERT INTO update_logs VALUES(null,"Updated Successfully in PLAYER SALARY Table",NOW())
//
DELIMITER ;
